...
# Helper to remove a method entirely by finding its bounds
def extract_method(content, method_start_str, include_comment=False):
    start = content.find(method_start_str)
...
# 2. Remove child: _eliteBadge(),
content = content.replace('child: _eliteBadge(),', '')

# 3. Remove _eliteBadge() method
content, _ = extract_method(content, '  Widget _eliteBadge() {')

# 4. Remove _vendorOnboardingCard method
content, _ = extract_method(content, '  Widget _vendorOnboardingCard(')

# 5. Remove _buildGrowthSection and inject new methods there
growth_start = content.find('  Widget _buildGrowthSection(')
if growth_start != -1:
...
"""
insert_pos2 = content.find('  Widget _buildAiSupportState(')
content = content[:insert_pos2] + fit_profile_replacement + content[insert_pos2:]

# 7. Replace _buildLegalPolicyEntry
legal_target = """  Widget _buildLegalPolicyEntry(BuildContext context) {"""
content, _ = extract_method(content, legal_target)
...
"""
insert_pos3 = content.find('  Widget _buildAiSupportState(')
content = content[:insert_pos3] + legal_replacement + content[insert_pos3:]


# 8. Modify the Profile Completion Progress bar inside _buildProfileHeaderCard
...
# 9. Extract AI Builder and Logout, and reconstruct slivers List
