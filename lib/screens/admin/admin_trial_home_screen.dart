...
class _AdminTrialHomeScreenState extends State<AdminTrialHomeScreen> {
            ...
            child: ListView(
              children: [
                ...
                _InfoRow(
                  label: 'Fee',
                  value:
                      '\u20B9${detail.trialFee.toStringAsFixed(0)} (${detail.paymentStatus})',
                ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Trial at Home')),
      body: _loading
                ...
                children: [
                  ...
                  if (_sessions.isEmpty)
                    const Padding(
                      ...
                      child: Center(
                        child: Text('No trial-home sessions for this filter.'),
                      ),
                    ),
                  ..._sessions.map(
                    (session) => Card(
  ...
  Widget _buildAnalytics(BuildContext context) {
    ...
    final activeTrials = _sessions
        .where((s) => s.isInProgress || s.isBooked)
        .length;
    final completedTrials = _sessions
        .where((s) => s.status == 'converted_to_order')
        .length;
    final awaitingPaymentCount = _sessions
        .where((s) => s.status == 'awaiting_final_payment')
        .length;
    ...
    final noShowPercentage = totalBooked > 0
        ? (noShowCount / totalBooked) * 100
        : 0.0;
    final inventoryReservedCount = _sessions
        .where(
          (s) =>
              s.status == 'booked' ||
              s.status == 'rider_assigned' ||
              s.status == 'out_for_trial_delivery' ||
              s.status == 'arrived' ||
              s.status == 'trial_in_progress' ||
              s.status == 'awaiting_final_payment',
        )
        .fold<int>(0, (sum, s) => sum + s.items.length);
    final codOrders = _sessions.where((s) => s.paymentStatus == 'cod').length;
    final onlineOrders = _sessions
        .where((s) => s.paymentStatus == 'online')
        .length;

    final conversionDenominator = totalBooked - cancelledCount - noShowCount;
    final conversionRate = conversionDenominator > 0
        ? (completedTrials / conversionDenominator) * 100
        : 0.0;

    final totalRevenue = _sessions
        .where((s) => s.status == 'converted_to_order')
        .fold<double>(0, (sum, s) => sum + s.trialFee);
    final aov = completedTrials > 0 ? totalRevenue / completedTrials : 0.0;
    final revenuePerTrial = completedTrials > 0
        ? totalRevenue / completedTrials
        : 0.0;
          ...
          Wrap(
            ...
            children: [
              ...
              _StatItem(
                ...
                value: '$inventoryReservedCount items',
              ),
              ...
              _StatItem(
                label: 'Revenue/Trial',
                value: '₹${revenuePerTrial.toStringAsFixed(0)}',
              ),
            ],
          ),
        ],
