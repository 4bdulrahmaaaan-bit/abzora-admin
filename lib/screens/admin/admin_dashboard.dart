...
class _AdminDashboardState extends State<AdminDashboard> {
    ...
    if (!auth.isSuperAdmin) {
      return const Scaffold(
        body: AbzioEmptyCard(
          ...
          subtitle:
              'This control center is restricted to platform administrators.',
        ),
      );
    }
    return Listener(
      ...
      child: Scaffold(
        ...
        body: _loading
            ? const AbzioLoadingView(
                ...
                subtitle:
                    'Preparing platform analytics, payouts, and store intelligence.',
              )
            : RefreshIndicator(
                onRefresh: _load,
                child: ListView(
                  ...
                  children: [
                        ...
                        child: width < 380
                            ? Column(
                                ...
                                children: [
                                  ...
                                  const Text(
                                    'Manage vendors, shops, banners, featured stores, and platform analytics',
                    ...
                    Builder(
                      builder: (context) {
                        final dailySales =
                            _analytics?.dailySales ?? const <AnalyticsPoint>[];
                        final todayRevenue = dailySales.isNotEmpty
                            ? dailySales.last.value
                            : 0.0;
                        final yesterdayRevenue = dailySales.length > 1
                            ? dailySales[dailySales.length - 2].value
                            : 0.0;
                        final activeRiders = _users
                            .where(
                              (u) => hasRiderOperationsAccess(u) && u.isActive,
                            )
                            .length;
                        ...
                        final delayedOrders = _orders
                            .where(
                              (o) => o.status.toLowerCase().contains('delay'),
                            )
                            .length;
                        ...
                        final cards = <_OpsHeroMetric>[
                          _OpsHeroMetric(
                            ...
                            value: '$activeOrders',
                            trend: _trendPercent(
                              activeOrders.toDouble(),
                              ((_analytics?.ordersToday ?? 0) - activeOrders)
                                  .abs()
                                  .toDouble(),
                            ),
                            color: Colors.blue,
                          ...
                          _OpsHeroMetric(
                            ...
                            value: '$_pendingKyc',
                            trend: _trendPercent(
                              _pendingKyc.toDouble(),
                              (_pendingKyc + 2).toDouble(),
                            ),
                            color: _pendingKyc > 0
                                ? Colors.orange
                                : Colors.teal,
                          ...
                          _OpsHeroMetric(
                            ...
                            value: '$activeRiders',
                            trend: _trendPercent(
                              activeRiders.toDouble(),
                              (activeRiders - 1).clamp(0, 99999).toDouble(),
                            ),
                            color: Colors.teal,
                          ...
                          _OpsHeroMetric(
                            ...
                            value: '$delayedOrders',
                            trend: _trendPercent(
                              delayedOrders.toDouble(),
                              (delayedOrders + 1).toDouble(),
                            ),
                            color: delayedOrders > 0
                                ? Colors.red
                                : Colors.green,
                          ...
                          _OpsHeroMetric(
                            ...
                            trend: _trendPercent(
                              highDemandZones.toDouble(),
                              (highDemandZones - 1).clamp(0, 99999).toDouble(),
                            ),
                        ...
                        return GridView.builder(
                          ...
                          itemBuilder: (context, index) =>
                              _OpsHeroMetricCard(metric: cards[index]),
                        );
                      },
                    ),
                    const SizedBox(height: 24),
                      ...
                      child: Column(
                        children: [
                          _FeatureTile(
                            ...
                            onChanged: (value) =>
                                _toggleSetting(field: 'custom', value: value),
                          ),
                          _FeatureTile(
                            label: 'Reels',
                            value: _settings.reelsEnabled,
                            onChanged: (value) =>
                                _toggleSetting(field: 'reels', value: value),
                          ),
                          _FeatureTile(
                            label: 'Offers',
                            value: _settings.offersEnabled,
                            onChanged: (value) =>
                                _toggleSetting(field: 'offers', value: value),
                          ),
                          _FeatureTile(
                            label: 'Checkout',
                            value: _settings.checkoutEnabled,
                            onChanged: (value) =>
                                _toggleSetting(field: 'checkout', value: value),
                          ),
                          _FeatureTile(
                            label: 'Marketplace',
                    ...
                    if (_disputes.isEmpty)
                      const AbzioEmptyCard(
                        ...
                        subtitle:
                            'Refund and dispute requests will appear here for admin review.',
                      )
                    else
                      ..._disputes.map(
                        (dispute) => Card(
                    ...
                    GridView.count(
                      ...
                      children: [
                        _AdminMetric(
                          label: 'Orders Today',
                          value: '${_analytics?.ordersToday ?? 0}',
                          icon: Icons.today_outlined,
                          color: Colors.green,
                        ),
                        _AdminMetric(
                          label: 'Revenue',
                          value: '₹${_analytics?.totalRevenue.toInt() ?? 0}',
                          icon: Icons.payments_outlined,
                          color: AbzioTheme.accentColor,
                        ),
                        _AdminMetric(
                          label: 'Commission',
                          value:
                              '₹${_analytics?.platformCommissionRevenue.toInt() ?? 0}',
                          icon: Icons.account_balance_outlined,
                          color: Colors.blue,
                        ),
                        _AdminMetric(
                          label: 'Vendor Payouts',
                          value: '₹${_analytics?.vendorPayouts.toInt() ?? 0}',
                          icon: Icons.store_outlined,
                          color: Colors.orange,
                        ),
                        _AdminMetric(
                          label: 'Rider Payouts',
                          value: '₹${_analytics?.riderPayouts.toInt() ?? 0}',
                          icon: Icons.delivery_dining_outlined,
                          color: Colors.purple,
                        ),
                        _AdminMetric(
                            ...
                            Wrap(
                              ...
                              children: [
                                _FinancePill(
                                  ...
                                  value: '₹${_finance!.vendorPending.toInt()}',
                                ),
                                _FinancePill(
                                  label: 'Rider Pending',
                                  value: '₹${_finance!.riderPending.toInt()}',
                                ),
                                _FinancePill(
                                  label: 'Pending Withdrawals',
                                  value:
                                      '₹${_finance!.pendingWithdrawalAmount.toInt()}',
                                ),
                                _FinancePill(
                                  label: 'Flagged Users',
                                  value: '${_finance!.flaggedUsers}',
                                ),
                                _FinancePill(
                                  label: 'Fraud Alerts',
                                  value: '${_finance!.fraudAlerts.length}',
                                ),
                              ],
                            ),
                          ],
                      ...
                      SizedBox(
                        ...
                        child: Row(
                          ...
                          children: _analytics!.dailySales.map((point) {
                            final max = _analytics!.dailySales.fold<double>(
                              1,
                              (value, item) =>
                    ...
                    _AdminAction(
                      ...
                      subtitle: 'Verify vendors and manage user accounts',
                    ...
                    _AdminAction(
                      title: 'Shop Control',
                      subtitle:
                          'Approve, reject, activate, feature, and isolate marketplace shops',
                      icon: Icons.verified_user_outlined,
                      color: Colors.orange,
                    ...
                    _AdminAction(
                      ...
                      subtitle:
                          'Manage products, categories, and platform merchandising',
                    ...
                    _AdminAction(
                      title: 'Order Control',
                      subtitle: 'Update order status and oversee fulfillment',
                    ...
                    _AdminAction(
                      title: 'Trial at Home',
                      subtitle:
                          'Monitor sessions, delivery progress, and conversion outcomes',
                    ...
                    _AdminAction(
                      ...
                      subtitle:
                          'Connected to live marketplace services. Refresh to pull latest operational state.',
                    ...
                    _AdminAction(
                      ...
                      subtitle:
                          'Monitor invoice queues, replay DLQ, and inspect delivery logs',
                      icon: Icons.receipt_long_outlined,
                      ...
                      ..._analytics!.topStores.map(
                        (store) => Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text(
                              'Wallet balance ₹${store.walletBalance.toInt()} | Commission ${(store.commissionRate * 100).toInt()}%',
                            ),
  ...
  void _openTab(BuildContext context, int tabIndex) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => AdminManagementScreen(initialTab: tabIndex),
      ),
    );
  }
}
