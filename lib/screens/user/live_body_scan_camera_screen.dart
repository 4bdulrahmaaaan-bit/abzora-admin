...
class _LiveBodyScanCameraScreenState extends State<LiveBodyScanCameraScreen>
  ...
  final List<PoseRefinementResult> _recentRefinements =
    ...
    try {
      ...
      Navigator.pop(
        ...
        LiveBodyScanCapture(
          ...
          sampleCount: _recentRefinements.length,
        ),
      );
    } catch (error) {
      if (!mounted) {
...
class _BodySilhouettePainter extends CustomPainter {
  const _BodySilhouettePainter({required this.pulse, required this.feedback});
...
class _CameraHintChip extends StatelessWidget {
  const _CameraHintChip({required this.label});
