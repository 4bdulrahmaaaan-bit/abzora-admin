...
class _VendorWorkspaceScreenState extends State<VendorWorkspaceScreen> {
  ...
  Future<void> _applyGrowthSuggestion(Map<String, dynamic> rec) async {
    ...
    if (perf.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Product data unavailable for this suggestion.'),
        ),
      );
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Applied suggestion to $productName at ${m(targetPrice)}.',
        ),
      ),
    );
    await _load();
  ...
  Future<void> _completeTrainingModule(
    CustomVendorTrainingModule module,
  ) async {
    final actor = _actor;
    if (actor == null) return;
  ...
  Future<void> _logout() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'You will need to sign in again to access your vendor workspace.',
        ),
        actions: [
          TextButton(
  ...
  Widget _home() {
    final a = _analytics!;
    final w = _wallet!;
    final customOrders = _orders
        .where((order) => order.fulfillmentType == 'custom_tailoring')
        .toList();
    final todayTailoringOrders = customOrders.where((order) {
      final created =
          DateTime.tryParse(order.createdAt ?? '') ?? order.timestamp;
    ...
    return ListView(
      ...
      children: [
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  const Expanded(child: Text('Accepting Orders')),
                  Switch(
            ...
            ? _MetricGrid(
                items: [
                  (
                    'Today Orders',
                    'New custom requests',
                    '$todayTailoringOrders',
                    const Color(0xFF374151),
                  ),
                  (
                    'In Production',
                    'Accepted or stitching',
                    '$inProduction',
                  ...
                  (
                    'Completed',
                    'Delivered custom pieces',
                    '${customOrders.where((o) => o.customOrderStatus == 'delivered').length}',
                    const Color(0xFF374151),
                  ),
                ],
              )
            : _MetricGrid(
                items: [
                  ...
                  (
                    'Completed Orders',
                    'Delivered',
                    '${_orders.where((o) => o.status == 'Delivered' || o.status == 'Completed').length}',
                    const Color(0xFF374151),
                  ),
                ],
              ),
        ...
        _MetricGrid3(
          items: [
            ('Commission Today', m(a.todayCommission), const Color(0xFFB91C1C)),
            ('Pending Settlement', m(w.pendingAmount), const Color(0xFFC2410C)),
            ('Available Balance', m(w.balance), const Color(0xFF15803D)),
          ],
        ),
        if (_isCustomVendor) ...[
          const SizedBox(height: 10),
          _Card(
            child: Column(
              ...
              children: [
                ...
                _line(
                  'On-time delivery',
                  '${((1 - _store!.customVendorProfile.metrics.delayRate) * 100).toStringAsFixed(0)}%',
                ),
                _line(
                  'Order success rate',
                  '${(_store!.customVendorProfile.metrics.orderSuccessRate * 100).toStringAsFixed(0)}%',
                ),
                _line(
                  'Production time',
                  '${_store!.customVendorProfile.productionTimeDays} days',
                ),
              ],
            ),
          ),
            ...
            _Card(
              child: Column(
                ...
                children: [
                  ...
                  _line(
                    'Quality score',
                    '${_qualityState!.quality.qualityScore.toStringAsFixed(0)}/100',
                  ),
                  _line(
                    'Visibility tier',
                    _qualityState!.quality.visibilityTier,
                  ),
                  _line(
                    'Training status',
                    _qualityState!.training.trainingStatus.replaceAll('_', ' '),
                  ),
                  _line(
                    'Sample approval',
                    _qualityState!.sampleReview.status.replaceAll('_', ' '),
                  ),
                  _line(
                    'Fit success',
                    '${(_qualityState!.quality.fitSuccessRate * 100).toStringAsFixed(0)}%',
                  ),
                  const SizedBox(height: 10),
                  Wrap(
                    spacing: 8,
              ...
              children: [
                ...
                if (customOrders.isEmpty)
                  const AbzioEmptyCard(
                    ...
                    subtitle:
                        'Accepted orders will appear here for stitching and QC.',
                  )
                else
                  ...customOrders
                ...
                children: [
                  _Action(
                    onTap: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => AddProductScreen(storeId: _store!.id),
                      ),
                    ),
                  ...
                  _Action(
                    ...
                    t: _isCustomVendor ? 'Custom Orders' : 'View Orders',
                    s: _isCustomVendor
                        ? 'Stitching workflow'
                        : 'Process quickly',
                  ),
                  ...
                  _Action(
                    onTap: () => Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (_) =>
                            PricingManagementScreen(storeId: _store!.id),
                      ),
                    ),
  ...
  Widget _ordersTab() {
    final filtered = _orders.where((o) {
      if (_isCustomVendor && o.fulfillmentType == 'custom_tailoring') {
        ...
        if (_orderTab == 'ready') {
          return o.customOrderStatus == 'ready' ||
              o.customOrderStatus == 'shipped';
        }
    ...
    return ListView(
      ...
      children: [
        _Card(
          child: Row(
            children: [
              _OChip(
                label: 'New',
                selected: _orderTab == 'new',
                onTap: () => setState(() => _orderTab = 'new'),
              ),
              _OChip(
                label: 'Processing',
                selected: _orderTab == 'processing',
                onTap: () => setState(() => _orderTab = 'processing'),
              ),
              _OChip(
                label: 'Ready',
                selected: _orderTab == 'ready',
                onTap: () => setState(() => _orderTab = 'ready'),
              ),
              _OChip(
                label: 'Completed',
                selected: _orderTab == 'completed',
                onTap: () => setState(() => _orderTab = 'completed'),
              ),
            ],
          ),
        ),
        ...
        if (_orderTab != 'completed')
          FilledButton.tonal(
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) =>
                    OrderManagementScreen(actor: _actor!, store: _store!),
              ),
            ),
  ...
  Widget _growthDashboard() {
    ...
    final conversionRate =
        ((_growthSummary['conversionRate'] ??
                    (a.orders == 0
                        ? 0.0
                        : (a.ordersCompleted / a.orders) * 100))
                as num)
            .toDouble();
    final avgOrderValue =
        ((_growthSummary['avgOrderValue'] ??
                    (a.ordersCompleted == 0
                        ? 0.0
                        : a.totalSales / a.ordersCompleted))
                as num)
            .toDouble();
    final revenue = ((_growthSummary['revenue'] ?? a.totalSales) as num)
        .toDouble();
    final completedOrders =
        ((_growthSummary['orders'] ?? a.ordersCompleted) as num).toInt();
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 2),
                    Text(
                      'Smart insights to grow your sales',
                  ...
                  (item) => Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          m(((item['currentPrice'] ?? 0) as num).toDouble()),
                          style: const TextStyle(fontWeight: FontWeight.w700),
                        ),
                        const SizedBox(width: 8),
                        Text(
                          item['suggestedAction']?.toString() ?? 'Review',
                          style: const TextStyle(
                            fontSize: 12,
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Demand insights: Sneakers trending up | Peak sales: 7-10 PM',
                  style: TextStyle(color: Colors.black54),
                ),
              ],
            ),
  ...
  Widget _earnings() {
    ...
    final tx = switch (_txFilter) {
      ...
      'withdrawals' =>
        allTx
            .where((e) => e.type == 'withdrawal' || e.type == 'payout')
            .toList(),
    ...
    final nextPayout = DateFormat(
      'EEE, d MMM',
    ).format(DateTime.now().add(const Duration(days: 1)));
    final payoutProcessing = w.withdrawalRequests.any(
        ...
        _Card(
          child: Column(
            ...
            children: [
              ...
              GridView.count(
                ...
                children: [
                  ...
                  _MoneyKpi(
                    ...
                    value: m(w.pendingAmount),
                    trend: '-1.3%',
                  ),
                  _MoneyKpi(
                    title: 'Today Earnings',
                    value: m(a.todayEarnings),
                    trend: trendUp ? 'up' : 'down',
                  ),
                  _MoneyKpi(
                    title: 'Weekly Earnings',
                    value: m(a.weeklyRevenue),
                    trend: '+9.1%',
                  ),
                ],
              ),
              ...
              InkWell(
                onTap: () => setState(
                  () => _showEarningsBreakdown = !_showEarningsBreakdown,
                ),
              ...
              AnimatedCrossFade(
                ...
                crossFadeState: _showEarningsBreakdown
                ...
                firstChild: Column(
                  children: [
                    ...
                    _line(
                      ...
                      m(a.weeklyCommission + a.todayCommission),
                    ),
                  ],
                ),
            ...
            children: [
              ...
              if (tx.isEmpty)
                const AbzioEmptyCard(
                  ...
                  subtitle: 'Transactions will show after activity.',
                ),
        ...
        _Card(
          child: Column(
            ...
            children: [
              ...
              _line(
                'Commission',
                '${(store.commissionRate * 100).toStringAsFixed(1)}%',
              ),
              _line(
                'Payout Profile',
                wallet.payoutProfile.isConfigured ? 'Configured' : 'Pending',
              ),
          ...
          _Card(
            child: Column(
              ...
              children: [
                ...
                _line(
                  'Experience',
                  '${store.customVendorProfile.experienceYears} years',
                ),
                _line(
                  'Specialization',
                  store.customVendorProfile.specializations.isEmpty
                      ? 'Not set'
                      : store.customVendorProfile.specializations.join(', '),
                ),
                _line(
                  'Price range',
                  '${m(store.customVendorProfile.priceRangeMin)} - ${m(store.customVendorProfile.priceRangeMax)}',
                ),
                _line(
                  'Production time',
                  '${store.customVendorProfile.productionTimeDays} days',
                ),
                _line(
                  'Alterations',
                  store.customVendorProfile.supportsAlterations
                      ? 'Supported'
                      : 'Not supported',
                ),
              ],
            ...
            _Card(
              child: Column(
                ...
                children: [
                  ...
                  _line(
                    'On-time delivery',
                    '${(_qualityState!.quality.onTimeDeliveryRate * 100).toStringAsFixed(0)}%',
                  ),
                  _line(
                    'Fit success',
                    '${(_qualityState!.quality.fitSuccessRate * 100).toStringAsFixed(0)}%',
                  ...
                  if (_qualityState!.sampleReview.adminFeedback.isNotEmpty)
                    _line(
                      'Admin feedback',
                      _qualityState!.sampleReview.adminFeedback,
                    ),
                ],
              ),
            ),
          ...
          child: Column(
            children: [
              ListTile(
                ...
                subtitle: const Text('Catalogue and stock'),
              ...
              ListTile(
                ...
                title: const Text('Store Controls'),
                subtitle: const Text('Branding and operations'),
                trailing: const Icon(Icons.chevron_right),
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
  ...
  Widget _line(String l, String v) => Padding(
    ...
    child: Row(
      children: [
        Expanded(
          child: Text(l, style: const TextStyle(color: Colors.black54)),
        ),
...
class _MoneyKpi extends StatelessWidget {
  const _MoneyKpi({required this.title, required this.value, this.trend = ''});
        ...
        children: [
          ...
          Align(
            ...
            child: FilledButton.tonal(
              onPressed: recommendationMap.isEmpty
                  ? null
                  : () => onApply(recommendationMap),
              child: Text(cta),
            ),
          ),
        ],
...
class _PriceConversionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final prices = sorted
        .map((e) => ((e['price'] ?? 0) as num).toDouble())
        .toList();
    final conv = sorted
        .map((e) => ((e['conversionRate'] ?? 0) as num).toDouble())
        .toList();
    ...
    for (var i = 0; i < sorted.length; i++) {
      ...
      final y =
          size.height - ((((conv[i] - minC) / dy) * (size.height - 12)) + 6);
      if (i == 0) {
        path.moveTo(x, y);
      } else {
        path.lineTo(x, y);
  ...
  Widget build(BuildContext context) => _Card(
    child: Column(
      ...
      children: [
        ...
        Text(
          'Customer: ${order.shippingLabel.isEmpty ? order.userId : order.shippingLabel}',
        ),
        Text(
        ...
        Text(
          'Commission: ${money(order.platformCommission)}',
          style: const TextStyle(
        ...
        Text(
          'You earn: ${money(order.vendorEarnings)}',
          style: const TextStyle(
            color: Color(0xFF15803D),
            fontWeight: FontWeight.w700,
          ),
          ...
          children: [
            Expanded(
              child: OutlinedButton(
                onPressed: order.status == 'Placed'
                    ? () => onSet(order, 'Confirmed')
                    : null,
            ...
            Expanded(
              child: FilledButton.tonal(
                onPressed: order.status == 'Packed'
                    ? () => onSet(order, 'Ready for pickup')
                    : null,
...
class _CustomOrderCard extends StatelessWidget {
  ...
  String _deliveryLabel(OrderModel order) {
    ...
    final days = order.customProductionTimeDays > 0
        ? order.customProductionTimeDays
        : 7;
          ...
          children: [
                  ...
                  _Card(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 6),
                        Text(
                          'Earnings per order: ${money(order.vendorEarnings)}',
                          style: const TextStyle(
                            color: Color(0xFF15803D),
                            fontWeight: FontWeight.w700,
                          ),
                        ...
                        Text(
                          'Delivery date: ${DateFormat('d MMM y').format((DateTime.tryParse(order.createdAt ?? '') ?? order.timestamp).add(Duration(days: order.customProductionTimeDays > 0 ? order.customProductionTimeDays : 7)))}',
                        ),
                      ],
                    ),
                  ),
            ...
            Wrap(
              ...
              children: [
                ...
                FilledButton(
                  onPressed: order.customOrderStatus == 'accepted'
                      ? () => onSet('in_stitching')
                      : null,
                  child: const Text('Start stitching'),
                ),
                FilledButton.tonal(
                  onPressed: order.customOrderStatus == 'in_stitching'
                      ? () => onSet('quality_check')
                      : null,
                  child: const Text('Mark QC done'),
                ),
                FilledButton.tonal(
                  onPressed: order.customOrderStatus == 'quality_check'
                      ? () => onSet('ready')
                      : null,
...
class _TxRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusText = e.status.isEmpty
        ? 'Completed'
