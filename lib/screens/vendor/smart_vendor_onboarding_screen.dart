...
class _SmartVendorOnboardingScreenState extends State<SmartVendorOnboardingScreen> {
  ...
  static const _steps = <String>['Store Info', 'Bank Details', 'Add Products', 'Enable Delivery'];
  ...
  Widget build(BuildContext context) {
    ...
    final progress = _step >= 4 ? 1.0 : (_step + 1) / 5;
    return Scaffold(
      backgroundColor: bg,
          ...
          children: [
            if (_step < 4) ...[
              Align(
                ...
                child: Text('Setup your store - ${((progress) * 100).round()}% complete', style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.w700)),
              ),
              const SizedBox(height: 10),
              ClipRRect(
                borderRadius: BorderRadius.circular(8),
                child: LinearProgressIndicator(value: progress, minHeight: 10, valueColor: const AlwaysStoppedAnimation(gold), backgroundColor: const Color(0xFFECE2CF)),
              ),
              const SizedBox(height: 10),
              Wrap(
                spacing: 8,
                ...
                children: List.generate(_steps.length, (i) {
                  ...
                  return Chip(
                    ...
                    backgroundColor: done ? const Color(0xFFEAF5EC) : active ? const Color(0xFFFFF7E5) : Colors.white,
                  );
                }),
              ),
            ],
  ...
  Widget _stepCard({
    ...
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: const Color(0xFFE8DFCF))),
      child: ListView(
        children: [
          ...
          for (final field in fields) ...[
            TextField(decoration: InputDecoration(labelText: field, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
            const SizedBox(height: 12),
          ],
          for (final tip in tips) ...[
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(color: const Color(0xFFFFF7E5), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFEAD7AA))),
              child: Text(tip, style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: const Color(0xFF7A5A00))),
            ),
            const SizedBox(height: 8),
          ],
          Text('✔ Step completed will auto-save instantly', style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: const Color(0xFF1C8C4E))),
        ],
      ),
    );
  }
  ...
  Widget _deliveryStep() {
    return _stepCard(
      ...
      subtitle: 'Auto-detect zone and suggest radius',
      fields: const ['Delivery Zone', 'Delivery Radius (km)'],
      tips: const ['Enable same-day delivery to increase conversions.'],
    );
  }
  ...
  Widget _finalState() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFFE8DFCF))),
      child: Column(
