...
class VendorNotificationApi {
  ...
  Future<Map<String, dynamic>> getNotifications({int page = 1, int limit = 20, String? priority, bool? unreadOnly}) async {
    final queryParameters = <String, String>{
      'page': page.toString(),
      'limit': limit.toString(),
    };
    if (priority != null) queryParameters['priority'] = priority;
    if (unreadOnly != null && unreadOnly) queryParameters['unreadOnly'] = 'true';
  ...
  Future<int> getUnreadCount() async {
    final payload = await _backendApiClient.get('/vendor/notifications/unread-count', authenticated: true);
  ...
  Future<void> markAsRead(String id) async {
    ...
    try {
      await _backendApiClient.post('/vendor/notifications/$id/read', authenticated: true, body: {'_method': 'PATCH'});
    } catch (_) {}
  }

  Future<void> markAllAsRead() async {
    try {
      await _backendApiClient.post('/vendor/notifications/read-all', authenticated: true, body: {'_method': 'PATCH'});
    } catch (_) {}
  }
}
