...
class GarmentBodyAlignmentEngine {
  ...
  GarmentAlignmentSnapshot compute({
    ...
    final shoulderRatio =
        (frame.shoulderWidth / shoulderBaseline).clamp(0.84, 1.46);
    ...
    final chestScale =
        ((shoulderScale * 0.78) + (torsoRatio * 0.22)).clamp(0.9, 1.34);
    final torsoScale = ((torsoRatio * 0.8) +
        ...
        .clamp(0.88, 1.34);
    final waistScale =
        ((torsoScale * 0.72) + (silhouetteBias * 0.42) + 0.18).clamp(0.72, 1.22);
    ...
    final chestCenterX = (frame.shoulderCenter.x +
            ((frame.hipCenter.x - frame.shoulderCenter.x) * 0.18))
        .clamp(0.0, 1.0);
    final chestCenterY = (shoulderCenterY + ((hipCenterY - shoulderCenterY) * 0.30))
        .clamp(0.0, 1.0);
    ...
    final centerX = (frame.shoulderCenter.x + (torsoSlopeX * 0.30))
        .clamp(0.0, 1.0);
    final waistX =
    ...
    final stabilized = GarmentAlignmentSnapshot(
      ...
      shoulderSlopeRadians: blend(previous.shoulderSlopeRadians, computed.shoulderSlopeRadians),
      ...
      waistTaperFactor: blend(previous.waistTaperFactor, computed.waistTaperFactor),
