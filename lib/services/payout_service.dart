...
class PayoutService {
  PayoutService({DatabaseService? databaseService})
  ...
  Future<VendorPayoutSummary> getVendorSummary({
    ...
    final orders = (await _databaseService.getAllOrders(
      actor: actor,
    )).where((order) => order.storeId == storeId).toList();
    final readyOrders = orders
        .where(
          (order) =>
              order.payoutStatus == 'Ready' || order.payoutStatus == 'Pending',
        )
        .toList();
    ...
    final netPaidOut = payouts.fold<double>(
      0,
